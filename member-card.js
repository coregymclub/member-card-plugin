...
const MemberCard = {
  ...
  renderAccessGrid(canOpen) {
    return Object.entries(this.GYM_NAMES).map(([key, name]) => {
      const hasAccess = canOpen[key] === true;
      return `
        <div class="mc-access-item ${hasAccess ? 'mc-access-granted' : 'mc-access-denied'}">
          <span class="mc-access-icon">${hasAccess ? '&#128994;' : '&#128308;'}</span>
          <span class="mc-access-name">${name}</span>
        </div>
      `;
    }).join('');
  },
